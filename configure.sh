...
#
# Check whether --enable-rdtsc was given.
if test "${enable_rdtsc+set}" = set; then :
  enableval=$enable_rdtsc;
		enable_rdtsc=1
  ...
  withval=$with_mysql;
		if test "$withval" != "no" ; then
			if test ! -x "$withval" ; then
				as_fn_error $? "$withval is not an executable file" "$LINENO" 5
			fi
...
if test "$enable_64bit" = "no" ; then
	{ $as_echo "$as_me:${as_lineno-$LINENO}: checking whether pointers can be stored in ints (old code)" >&5
$as_echo_n "checking whether pointers can be stored in ints (old code)... " >&6; }
...
rm -f core conftest.err conftest.$ac_objext conftest.$ac_ext
	...
	{ $as_echo "$as_me:${as_lineno-$LINENO}: result: $pointers_fit_in_ints" >&5
$as_echo "$pointers_fit_in_ints" >&6; }
	if test "$pointers_fit_in_ints" = "no" ; then
		as_fn_error $? "pointers cannot be stored in ints, required for old code... stopping" "$LINENO" 5
	fi
fi
...
if test "$ac_res" != no; then :
  ...
  as_fn_error $? "math library not found... stopping" "$LINENO" 5
fi
...
if test "$ac_cv_search_clock_gettime" != "no" ; then
	{ $as_echo "$as_me:${as_lineno-$LINENO}: checking whether CLOCK_MONOTONIC is supported and works" >&5
$as_echo_n "checking whether CLOCK_MONOTONIC is supported and works... " >&6; }
	if test "$cross_compiling" = yes; then :
...
if ${ac_cv_path_MYSQL_CONFIG_HOME+:} false; then :
  ...
  [\\/]* | ?:[\\/]*)
  ac_cv_path_MYSQL_CONFIG_HOME="$MYSQL_CONFIG_HOME" # Let the user override the test with a path.
  ;;
  *)
  as_save_IFS=$IFS; IFS=$PATH_SEPARATOR
for as_dir in $PATH
...
IFS=$as_save_IFS
  ...
  test -z "$ac_cv_path_MYSQL_CONFIG_HOME" && ac_cv_path_MYSQL_CONFIG_HOME="no"
  ;;
esac
fi
MYSQL_CONFIG_HOME=$ac_cv_path_MYSQL_CONFIG_HOME
if test -n "$MYSQL_CONFIG_HOME"; then
  { $as_echo "$as_me:${as_lineno-$LINENO}: result: $MYSQL_CONFIG_HOME" >&5
$as_echo "$MYSQL_CONFIG_HOME" >&6; }
else
  { $as_echo "$as_me:${as_lineno-$LINENO}: result: no" >&5
$as_echo "no" >&6; }
fi
...
if test "$HAVE_MYSQL" = "yes" ; then
	{ $as_echo "$as_me:${as_lineno-$LINENO}: result: yes ($MYSQL_VERSION)" >&5
$as_echo "yes ($MYSQL_VERSION)" >&6; }
else
	{ $as_echo "$as_me:${as_lineno-$LINENO}: result: no" >&5
$as_echo "no" >&6; }
	as_fn_error $? "MySQL not found or incompatible" "$LINENO" 5
fi
